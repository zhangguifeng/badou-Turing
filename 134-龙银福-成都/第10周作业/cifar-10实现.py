...
# 创建一个variable_with_weight_loss()函数(生成卷积核kernel参数和全连接层权重参数)，该函数的作用是：
def variable_with_weight_loss(shape, stddev, w1):
    var = tf.Variable(tf.truncated_normal(shape, stddev=stddev))
    # 1-使用参数 w1 控制 L2 loss 的大小
    # 2-使用函数 tf.nn.l2_loss() 计算权重 L2 loss
    # 3-使用函数 tf.multiply() 计算权重 L2 loss 与 w1 的乘积，并赋值给 weights_loss
    # 4-使用函数 tf.add_to_collection() 将最终的结果放在名为 losses 的集合里面，方便后面计算神经网络的总体 loss
    if w1 is not None:
        weights_loss = tf.multiply(tf.nn.l2_loss(var), w1, name='weights_loss')
        tf.add_to_collection('losses', weights_loss)
    return var

# 使用上一个文件里已经定义好的文件序列读取函数读取训练数据和测试数据
...
# 注意：由于以后定义全连接网络的时候用到了batch_size，所以x中，第一个参数不应该是None，而应该是batch_size
...
# tf.nn.in_top_k() 用来计算输出结果中top_K的准确率，函数默认的K值是1，即top_1的准确率，也就是输出分类准确率最高时的数值
top_k_op = tf.nn.in_top_k(result, y_, 1)

init_op = tf.global_variables_initializer() # 初始化所有的 variable 变量
with tf.Session() as sess:
    sess.run(init_op)
    # 启动线程操作，这是因为之前数据增强的时候使用train.shuffle_batch()函数的时候通过参数num_threads()配置了16个线程用于组织 batch 的操作
    tf.train.start_queue_runners()
    # 每隔 100 step 会计算并展示当前的 loss, 每秒钟能训练的样本数量, 以及训练一个 batch 数量所花费的时间
    ...
    # 计算最终的正确率
    num_batch = int(math.ceil(num_examples_for_eval / batch_size)) # math.ceil() --> 向上取整，即小数部分直接舍去，并向正数部分进1
    true_count = 0
    total_sample_count = num_batch * batch_size

    # 在一个 for 循环里统计所有预测正确的样例个数
