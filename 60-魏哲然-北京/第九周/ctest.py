...
import time
import math
import cnn.Cifar.cdata as Cifar10_data
import tensorflow.compat.v1 as tf

tf.disable_v2_behavior()
max_steps = 400
batch_size = 8
num_examples_for_eval = 100
...
conv2 = tf.nn.conv2d(x, kernel1, [1, 1, 1, 1], padding="SAME")
...
#                 sec_per_batch = float(duration)
#                 print("step %d loss:%.2f(%.1f exps/sec %.3f sec/batch)" % (
#                     step, loss_value, exampels_per_sec, sec_per_batch))
#         num_batch = int(math.ceil(num_examples_for_eval / batch_size))
#         true_count = 0
#         total_sampe_count = num_batch * batch_size
#         for j in range(num_batch):
#             image_batch, label_batch = sess.run([images_test, labels_test])
#             predictions = sess.run([top_k_op], feed_dict={x: image_batch, y_: label_batch})
#             true_count += np.sum(predictions)
#         print("accuracy:%.3f" % ((true_count / total_sampe_count) * 100))
#
#     finally:
#         coord.request_stop()
#         coord.join(threads)
